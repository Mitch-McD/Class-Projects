...
BLOCK2 * toBlocks(char *array, int *oldhw, int *hw, int fileType) {
    BLOCK2 *blocks = (BLOCK2*) malloc((hw[0]/N)*(hw[1]/N) * sizeof(BLOCK2));
    int i, j, k = 0;
	...
	for (k = 0; k < blockAmount; k++) {
	    for (i = 0; i < N; i++) {
	    	for (j = 0; j < N; j++) {
    		    blocks[k].b[i][j] = array2d[i+N*(k/(hw[1]/N))][j+N*(k%(hw[1]/N))];
		}
	    }
	}
    }
	...
	for (i = 0; i < hw[0]; i++) {
        ...
        for (k = 0; k < blockAmount; k++) {
            for (i = 0; i < N; i++) {
                for (j = 0; j < N; j++) {
		    blocks[k].r[i][j] = arrayRGB[i+N*(k/(hw[1]/N))][j+N*(k%(hw[1]/N))][0];
                    blocks[k].g[i][j] = arrayRGB[i+N*(k/(hw[1]/N))][j+N*(k%(hw[1]/N))][1];
		    blocks[k].b[i][j] = arrayRGB[i+N*(k/(hw[1]/N))][j+N*(k%(hw[1]/N))][2];
		}
	    }
        }
    }
...
char * fromBlocks(BLOCK2 *blocks, int size, int *oldHW, int *newHW, int fileType) {
    ...
    for (k = 0; k < blockAmount; k++) {
        for (i = 0; i < N; i++) {
  	    for (j = 0; j < N; j++) {
	    	if (fileType == 5)
		    array2d[i+N*(k/(newHW[1]/N))][j+N*(k%(newHW[1]/N))] = blocks[k].b[i][j];
	    	else {
		    arrayRGB[i+N*(k/(newHW[1]/N))][j+N*(k%(newHW[1]/N))][0] = blocks[k].r[i][j];
		    arrayRGB[i+N*(k/(newHW[1]/N))][j+N*(k%(newHW[1]/N))][1] = blocks[k].g[i][j];
		    arrayRGB[i+N*(k/(newHW[1]/N))][j+N*(k%(newHW[1]/N))][2] = blocks[k].b[i][j];
		}
	    }
	}
    }
...
void printFirstLast(BLOCK2 *blocks, int *heightWidth, int *newHW, int fileType) {
    int i, j, k = 0;
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
	    if (fileType == 5)
                printf(" %d ", blocks[0].b[i][j]);
	    else
		printf(" %d ", blocks[0].r[i][j]);
        }
        printf("\n");
    }
    printf("Last Block ...\n");
    for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
	    if (fileType == 5)
        	printf(" %d ", blocks[blockAmount-1].b[i][j]);
            else
		printf(" %d ", blocks[blockAmount/3-1].r[i][j]);
        }
        printf("\n");
    }
