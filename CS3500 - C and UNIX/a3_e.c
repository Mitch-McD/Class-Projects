...
Storage * countConsecutive(int *data) {
    Storage *occOfVal = (Storage*)calloc(N*N,sizeof(Storage));
    int count = 1;
    int i;
    int x = 0;
    for (i = 0; i < N*N-1; i++) {
...
void convertToBits(Storage *occOfVal, FILE *fp, int avg, int blockCount) {
    ...
    while (occOfVal[i].occurrence != NULL) {
        size++;
        i++;
    }
    writeKBits(occOfVal, size, fp, avg,  blockCount);
}

void writeKBits(Storage *occOfVal, int dataSize, FILE *fp, int avg, int blockCount) {
    int i = 0;
    while ((17*dataSize+i)%8 != 0) {
        i++;
    }
    int byteSize = dataSize*17 + i + 8;
    int count = 0;
    int bytes[byteSize];
    i = 0;
    int j;
